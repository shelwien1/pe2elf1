...
#undef CB
  ...
  cost_t OptimizeGroupHost() {
    ...
    if( !cbuf )
      cbuf = new cost_t[size_t(MRP_MAXCLASS)*MAXC*MRP_GROUP*nu];
...
#define CB(cl, k, gr) (cbuf+((size_t(cl)*nc+(k))*MRP_GROUP+(gr))*nu)
  ...
  int CLInit() {
    if( !g_clopt.use )
      return 0;
    if( !g_cl.Open(g_clopt) ) {
      // Naming a device that is not there is a mistake worth stopping
      // for; not finding one when none was asked for is not.
    ...
    for( uint k = 0; k<nc; k++ )
      if( nt[k]>ntmax )
        ntmax = nt[k];
    size_t bn = size_t(H+PADT)*BS;
    if( !g_cl.Build(W, H, nc, BS, bn, ntmax, num_class, MRP_GROUP*num_pm*NUM_SUBPM) )
      return 0;
  ...
  cost_t OptimizePredictorCL() {
    ...
    static double* scan = 0;
    if( !scan )
      scan = new double[size_t(MRP_MAXCLASS)*CL_NCAND];
    for( uint k = 0; k<nc; k++ ) {
      int n = nt[k];
      if( n<2 )
        continue;
  ...
  cost_t OptimizeGroupCL() {
    ...
    if( opt_loop>1&&num_pm>1 ) {
      static double* pc2 = 0;
      if( !pc2 )
        pc2 = new double[size_t(MAXC)*MRP_GROUP*NUM_PMODEL];
      size_t npix = size_t(W)*H;
...
static BmpInfo ParseBMP(const byte* d, uint flen_) {
  ...
  if( isz<40||pl!=1||comp!=0 )
    return b;
  if( bpp!=24&&bpp!=32 )
    return b; // RGB / RGBA only
  if( w<=0||h==0 )
    return b;
  uint H_ = uint(h<0 ? -h : h), W_ = uint(w);
  if( off<HDR0||off>flen_ )
    return b;
  if( off<14+isz )
    return b;
...
static BC(*o1)[256];
static INLINE uint CodeByte(BC* t, uint c) {
  return CodeBits(t, c, 8);
}
...
int main(int argc, char** argv) {
  ...
  for( int i = 1; i<argc; i++ ) {
    ...
    switch( a[1] ) {
      ...
      case 'd':
        ...
        break;
      case 'p':
        g_clopt.plat = ArgNum(argc, argv, i, "-p");
        break;
      case 'T': {
        const char* t = a[2] ? a+2 : ((i+1<argc) ? argv[++i] : "");
        g_clopt.type = (t[0]=='c') ? 1 : (t[0]=='g') ? 2 : (t[0]=='a') ? 3 : 0;
        break;
      }
      default:
  ...
  if( list ) {
#ifdef MRP_OPENCL
    CLListDevices(stdout);
#else
    printf("this build has no OpenCL support (compile with -DMRP_OPENCL)\n");
#endif
    if( npos<3 )
      return 0;
  }
  ...
  if( npos<3 ) {
    printf("mrpc - lossless RGB/RGBA BMP compressor on MRP's scheme\n"
           ...
           argv[0], argv[0], MRP_MAXCLASS, PRD_ORDER, XPRD_ORDER);
    return 1;
  }
  ...
  if( f_DEC==0 ) {
    ...
    f_len = 0;
    if( fread(&f_len, 1, sizeof(f_len), f)!=sizeof(f_len) )
      return 4;
    b.ok = uint(getc(f))&1;
    rc.StartDecode(f);
  }
  ...
  if( b.ok ) {
    ...
    if( f_DEC==0 ) {
      if( fread(cd.img, 1, b.pixbytes, f)!=b.pixbytes )
        return 6;
    }

#if MRP_PROGRESS
    g_prog = (f_DEC==0)&&(MRP_PROGRESS>1||double(b.W)*double(b.H)>=MRP_PROGMIN);
#endif
    t_start = tnow();
    if( f_DEC==0 )
      cd.Encode();
