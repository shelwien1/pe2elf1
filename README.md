...
| `make coders` | Decodes `testfiles/coders/`, three images transcoded by `jpegtran` into sequential/progressive × Huffman/arithmetic, plus restart-interval versions of the arithmetic pair. Because the transcode is lossless, all six must report identical MCU and block counts — a disagreement is an entropy decoder bug. |
...
| `make coder` | Takes every bundled image apart into coefficients with `jpgcoder` and puts it back, requiring the result to equal the input byte for byte — and that *something* reached the coefficient form, since leaving every scan in the header is trivially lossless. 145 files, 223 of 318 scans transcoded, Huffman and arithmetic alike; almost all of the rest are scans of damaged files that stop part-way. Included in `make test`. |
