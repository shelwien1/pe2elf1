...
`REVIEW.md`, `alt_p1_block.inc`.  `update_model` was 533 lines and 121 pairs of
them were one counter bumped with the node's total kept in step:
...
**It stopped checking, silently.**  This anchored on the string
`int32_t update_model()` and walked forward from it.  A later round collapsed
that function's nine copy-pasted selector blocks into one method and a loop,
and made it return `void` -- so the anchor was gone, `next()` raised
`StopIteration`, and `sweep.sh` reported the tool as *failing* rather than as
finding nothing.  A crash reads like a defect in the tree; it was a defect
here.  It scans the whole file now: the pattern is a counter and its total on
consecutive lines, which is specific enough not to need a function to sit in,
and it cannot go stale when one is renamed.
