...
/// MODIFIED: as predict(), but over samples already in memory in BSQ order, so that a
/// caller which produced them itself does not have to spill them to a raw file first.
/// The samples stay owned by the caller.
