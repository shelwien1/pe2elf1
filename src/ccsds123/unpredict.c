...
int unpredict(input_feature_t input_params, predictor_config_t predictor_params, unsigned short int * residuals, char outputFile[128]){
    // MODIFIED: the reconstruction split away from the file writing, so a caller that
    // wants to keep building on the samples (e.g. to assemble a BMP) can have them.
    ...
    if(unpredict_samples(input_params, predictor_params, residuals, samples) != 0){
        free(samples);
        return -1;
    }
...
/// MODIFIED: the body of unpredict(), leaving the samples in the caller's buffer.
int unpredict_samples(input_feature_t input_params, predictor_config_t predictor_params,
    unsigned short int * residuals, unsigned short int * samples){
