...
long long mod_star(long long arg, long long op, int debug){
    // MODIFIED: this used to add 2^(R-1), shift the sum right by R and shift the
    // quotient back left, which shifts a negative value in both directions -- and at
    // R = 64 it also formed 2^63 in a signed long long.  Reducing modulo 2^R with
    // unsigned arithmetic and then sign extending picks the same representative, the
    // unique one in [-2^(R-1), 2^(R-1)), without any of that.
    unsigned long long mask, value, sign;
    if(op >= 64){
        // A long long is already its own residue modulo 2^64.
        return arg;
    }
    mask = (((unsigned long long)0x1) << op) - 1;
    sign = ((unsigned long long)0x1) << (op - 1);
    value = ((unsigned long long)arg) & mask;
    if((value & sign) != 0)
        value -= mask + 1;
...
unsigned int read_bits(FILE * compressedStream, unsigned int num_bits, unsigned char *buffer, unsigned int *buffer_len){
    ...
    while(to_read > *buffer_len){
        ...
        to_read -= *buffer_len;
        // MODIFIED: this used to print the read state to stderr on every miss, which
        // for a truncated stream is once per remaining sample.  Running off the end is
        // reported by the caller now, which knows how much was actually decoded.
