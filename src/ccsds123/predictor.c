    ...
    if(predictor_params.neighbour_sum != 0){
        if(y > 0){
            if(x > 0){
                ...
                // MODIFIED: the standard's neighbour oriented local sum reaches to
                // (x+1, y-1) here, which does not exist when the image is one column
                // wide.  Upstream read past the end of the row and landed on the very
                // sample being predicted -- the encoder saw the original value there
                // and the decoder saw a not yet reconstructed zero, so a 1 pixel wide
                // image did not survive a round trip.  The wider case is untouched;
                // the degenerate one falls back on the northern sample, exactly as the
                // last column of a wider image already does.
                if(x + 1 < input_params.x_size)
...
//             fprintf(stderr, "directional_difference[0]=%d, directional_difference[1]=%d, directional_difference[2]=%d\n", directional_difference[0], directional_difference[1], directional_difference[2]);
        ...
        // MODIFIED: same as above -- the local sum sits below 4*s_mid as often as not,
        // so this shifted a negative value.  The multiplication is the same arithmetic.
        scaled_predicted = mod_star(diff_predicted + (local_sum_temp - 4*(long long)s_mid)*(((long long)0x1) << predictor_params.weight_resolution), predictor_params.register_size, x == 1 && y == 0 && z == 0);
...
int predict(input_feature_t input_params, predictor_config_t predictor_params, char inputFile[128], unsigned short int * residuals){
    // MODIFIED: file parsing split away from the prediction itself, so a caller that
    // already holds the samples in memory (e.g. one that decoded a BMP) can predict
    // without a round trip through a raw file.
    int result = 0;
    unsigned short int * samples = (unsigned short int *)malloc(sizeof(unsigned short int)*input_params.x_size*input_params.y_size*input_params.z_size);
    if(samples == NULL){
    ...
    result = predict_samples(input_params, predictor_params, samples, residuals);
    free(samples);
    return result;
}
...
int predict_samples(input_feature_t input_params, predictor_config_t predictor_params,
    ...
    weights = (int *)malloc(sizeof(int)*(weights_len > 0 ? weights_len : 1));
    if(weights == NULL){
...
//                 fprintf(stderr, "sample=%d\n", MATRIX_BSQ_INDEX(samples, input_params, x, y, z));
    ...
    // Freeing allocated memory (the samples belong to the caller)
    if(weights != NULL){
        free(weights);
    }
