...
/// MODIFIED: as unpredict(), but leaving the reconstructed samples in the caller's
/// buffer (BSQ order, x_size*y_size*z_size entries) instead of writing them to a raw
/// file, so that a caller can go straight on to build an image out of them.
