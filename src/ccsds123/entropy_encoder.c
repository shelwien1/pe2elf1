...
void zero_block_code(input_feature_t input_params, encoder_config_t encoder_params,
    ...
    // MODIFIED: a run of five or more zero blocks ending on a segment boundary used to
    // be sent as the ROS code (an FS value of 4, "the rest of the segment is zero").
    // The decoder sizes ROS as min(rest of the segment, rest of the reference sample
    // interval), but this encoder never breaks a run at a reference interval -- the
    // test for it compares a counter that has already been reduced modulo the interval
    // against the interval itself, so it never fires.  The two disagreed whenever the
    // reference sample interval was not a multiple of the 64 block segment, and the
    // image decoded to noise from that point on.  The explicit count below says the
    // same thing in a handful more bits, is what this encoder already emitted away from
    // a segment boundary, and any conformant decoder reads it back unchanged.
    (void)end_of_segment;
...
int encode_to_buffer(input_feature_t input_params, encoder_config_t encoder_params, predictor_config_t predictor_params,
    ...
    *out_stream = NULL;
    ...
    // MODIFIED: the buffer used to be sized at the uncompressed size of the residuals,
    // on the assumption that coding never expands.  It can: a sample-adaptive escape
    // costs u_max + D bits, so worst case is 6 bytes per sample rather than 2, and a
    // pathological image would have written past the end of the buffer.  Sized for the
    // worst case instead, plus room for the header and the weight/accumulator tables.
    stream_size = (size_t)input_params.x_size*input_params.y_size*input_params.z_size;
    stream_size = stream_size*((encoder_params.u_max + input_params.dyn_range + 15)/8) + 65536
                + (size_t)input_params.z_size*8;
    compressed_stream = (unsigned char *)malloc(stream_size);
    if(compressed_stream == NULL){
        fprintf(stderr, "Error in the allocation of the compressed stream\n\n");
        return -1;
    }
    memset(compressed_stream, 0, stream_size);
    ...
    // MODIFIED: a stream ending mid-byte still owes the reader that byte.
    if(written_bits > 0)
        written_bytes++;

    if(written_bytes > stream_size){
        fprintf(stderr, "Error, the coded stream overran its buffer\n\n");
        free(compressed_stream);
        return -1;
    }

    // The work buffer is sized for the worst case, which is several times the size of
    // the original image; the result is handed back in a buffer of its own size so a
    // caller comparing several codings does not hold two of those at once.
    coded = (unsigned char *)malloc(written_bytes > 0 ? written_bytes : 1);
    if(coded == NULL){
        fprintf(stderr, "Error in the allocation of the compressed stream\n\n");
        free(compressed_stream);
        return -1;
    }
    memcpy(coded, compressed_stream, written_bytes);
    free(compressed_stream);
    *out_stream = coded;
...
int encode_to_stream(input_feature_t input_params, encoder_config_t encoder_params, predictor_config_t predictor_params,
    ...
    unsigned char * coded = NULL;
    int written_bytes = encode_to_buffer(input_params, encoder_params, predictor_params, residuals, &coded);
    int write_result = 0;

    if(written_bytes < 0)
        return -1;
    write_result = fwrite(coded, 1, written_bytes, outFile);
    free(coded);
    if(write_result != written_bytes){
        fprintf(stderr, "Error in writing the compressed stream: only %d bytes out of %d written\n\n", write_result, written_bytes);
        return -1;
    }
    return written_bytes;
}
...
int encode(input_feature_t input_params, encoder_config_t encoder_params, predictor_config_t predictor_params,
    ...
    written_bytes = encode_to_stream(input_params, encoder_params, predictor_params, residuals, outFile);
    fclose(outFile);
    return written_bytes;
}
