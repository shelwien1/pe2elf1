...
int main(int argc,char**argv){
  ...
  for(int V=0;V<7;V++){
    ...
    for(u32 b=0;b<NBYTE;b++){ ctx=one;
      for(int j=0;j<8;j++){
        ...
        if(V>=5){ // interleaved container: one contiguous load + a permute to
          ...
          if(V==6) v=_mm512_i32gather_epi32(_mm512_add_epi32(pos,strmbase),(const int*)strm,1);
        } else
        ...
        if(V){ __m512i k=_mm512_add_epi32(_mm512_add_epi32(st,st),_mm512_maskz_set1_epi32(m,1));
               ...
               else if(V>=5){ // arithmetic counter: p += (bit ? -p : (SCALE-p)) >> 5
    ...
    REPORT(V==0?"chain only, separate pp[] (no model write-back)"
          ...
          :V==5?"target: contiguous refill + arithmetic counter (1 gather)"
                :"  ... same, but the refill left as a gather",t);
