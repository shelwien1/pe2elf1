...
%CXX% -O2 timetest.cpp %LDFLAGS% -o timetest.exe || echo note: timetest did not build - test.bat will not run
exit /b 0
...
echo [%p% 3/3] %CXX% %CXXFLAGS% %p%.cpp coder1-%p%.s %LDFLAGS% -o %p%.exe
