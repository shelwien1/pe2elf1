# opt.pl -- parameter optimizer for paq8hpc (adapted from dxt5comp's, which was
# adapted from auto_opt2.pl)
#
...
# The build must therefore be the tuning build (./mk.sh with no argument);
# ./mk.sh release folds every pattern away and there is nothing left to patch.
#
#   ./mk.sh && perl IDX/opt.pl [corpus-file-list]
#
# corpus-file-list: text file, one input path per line (default: opt.lst, and if
# that is missing, the single file $deffile below). Optimizing on one file
# overfits it; a mixed corpus is a much better target -- and note that this
# model has separate word/sparse/record submodels, so a corpus of pure English
# prose leaves the others drifting with nothing measuring them.
#
# $level is paq8hpc's memory level and is NOT a tuning parameter -- it decides
# the table sizes, so a run at one level does not transfer to another.  Tune at
# the level you intend to ship.
#
...
sub measure {
  ...
  for my $f (@files) {
    unlink $tmp;
    system( "$exe c \"$f\" $tmp $level >/dev/null 2>&1" );
    my $s = -s $tmp;
...
undef $/;
open INP, "<$exe" or die "no $exe -- build the tuning version first (./mk.sh)\n";
