...
for bits in 32 64; do
  BMF_OUT="$tmp/bmf$bits" BMF_BITS=$bits ./build.sh >"$tmp/build$bits" 2>&1 || {
...
mkdir -p "$tmp/extra"
"$tmp/bmf32" c testfiles/t1.bmp  "$tmp/extra/arc2.bmf" >/dev/null 2>&1 &&
"$tmp/bmf32" c testfiles/t8g.bmp "$tmp/extra/arc2.bmf" >/dev/null 2>&1 || {
...
echo
echo "$count mutants of $(cut -f2 "$tmp/manifest" | sort -u | wc -l) seed files through both builds, seed $seed"
