...
// `#define BMF_STANDALONE 1` stood here, selecting the standalone half of
// bmfhead.h and of each body.  Both of those `#include`s are commented out
// below and nothing reads it any more.
...
// uint32_t — so rather than supply a second wrong set, the file declared
// `_WINDOWS_` and let there be none: a body that grew a `BYTE` back would then
// fail to compile instead of quietly sign-extending.  That worked by being
// seen by `windows.h`, which is not included here and never was; the bodies
// have no `BYTE` left and the guard was doing nothing.  Removed.
// ---------------------------------------------------------------------------
...
// `DWORD1`..`DWORD3` and `SDWORD1`..`SDWORD3` stood here, added because "the
// bodies also index DWORD lanes directly".  No body does any more, and the
// three signed ones never could have: they expand to `SDWORDn`, which this
// file has never defined.  Six macros, three of them broken, under a comment
// saying they were needed -- which is what an unused definition costs.
