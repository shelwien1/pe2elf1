...
subs1.hpp lines                      17787     17616         17557
bmf.cpp lines                            —         —           365
raw-offset sites                        22        12              0
  off `_this`                            —         1             0
  in functions                           —         1             0
byte offsets on a typed base           121         0             0
pointer casts                         2137      1545          1163
  to a scalar                            —         —           445
  to a record                            —         —           372
  to a scalar, of an address             —         —           341
...
structs                                 —         —            24
  still ObjN                             —         —             0
fNN members / named ones             93/121     5/162         0/168
...
goto / LABEL_n:                     112/79     81/55         49/33
  ...
  jump into a block                      —         —             0
conversion warnings                   1455      1331           945
```
...
### And why the replay is still slow

`unnamed.py`'s fast path is only for a file the map fits, so `proven.sh`'s
twelve replays each fall back to the two-minute search — three tools reach it,
which is most of an hour. `addrmap.txt` is committed, so `proven.sh` puts each
revision's own map beside its `subs1.hpp` and `unnamed.py` looks there first.

That helps less than it should, and the reason is worth writing down: the map
at `2127451` does not fit `2127451`'s file either. It names
`init_sse_constants` and `log_two_lane`, two bodies deleted after the last time
anyone regenerated it. `addrmap.txt` is generated by hand and lags the renames,
so most historical revisions have a map that is already stale — and the test
rejects it, correctly, and pays for the search.
