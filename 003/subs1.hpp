...
int32_t __choose_plane_coding(BmfImage *img, int32_t unused_h, int8_t unused_c)
{
  ...
  if ( n_planes > 0 )
  {
    ...
    result = next_plane - 1;
    if ( n_planes > (uint32_t)(next_plane - 1) )
    {
...
void __alt_p2_d8_encode_body(AltP2Block *blk, uint8_t *src, int32_t width, int32_t height, uint8_t *out)
{
  ...
  int32_t q, resid, drift, q2, Size, last, *r0, pred, resid2, drift2, code;
  int64_t err;
  P2Ctx *rec2;
  uint32_t k;
  uint8_t *src_end;
