...
struct RangeCoder {
  ...
  uint32_t encode() { return encode(cum, high, tot); }
  uint32_t encode(uint32_t cum_, uint32_t high_, uint32_t tot_) {
...
BMF_SSE int32_t __sub_413900(uint16_t *_this, int32_t n2) {
  ...
  n0x800000_6 = rc.range;
  n0x7F800000_6 = rc.low;
  if ( *_this > 0x4000u )
  {
