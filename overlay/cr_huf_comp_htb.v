...
begin
  ...
  begin
    ...
    /* work_table_entry/node_ptr/row_ptr_del reset moved to genvar generate */
    ...
    begin
       ...
       /* work_table_entry/node_ptr/row_ptr_del update moved to genvar generate */
...
// --- Verilator bring-up: per-replicator drive of the fanout-replicated state ---
// Equivalent to the original loops (identical value to every replicator); each
// element k is its own always_ff so there is no delayed write to a loop-indexed
// array (BLKLOOPINIT) and no '{default:0} on a 2-D unpacked array (CONST error).
genvar gk_wt;
generate
  for (gk_wt = 0; gk_wt < N_FANOUT_REPLICATORS; gk_wt++) begin : g_work_table
    always_ff @(posedge clk_gated or negedge rst_n)
      if (~rst_n) begin
        work_table_entry[gk_wt][0] <= '0;
        work_table_entry[gk_wt][1] <= '0;
        node_ptr[gk_wt]            <= '0;
        row_ptr_del[gk_wt]         <= '0;
      end else begin
        work_table_entry[gk_wt]    <= work_table_entry_c;
        node_ptr[gk_wt]            <= node_ptr_c;
        row_ptr_del[gk_wt]         <= row_ptr_del_c;
      end
  end
endgenerate
