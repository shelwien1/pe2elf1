...
begin
  ...
  begin
    eob_r <= MIDDLE;
    is_cur_st <= IDLE;
    /* sorted_inputs reset moved to genvar generate (Verilator BLKLOOPINIT) */
    ...
    begin
	    ...
	    /* sorted_inputs update moved to genvar generate (Verilator BLKLOOPINIT) */
...
// --- Verilator bring-up: per-replicator drive of sorted_inputs ---
// Equivalent to the original loop bodies (identical value to every replicator),
// but each element k is its own always_ff so there is no delayed write to a
// loop-indexed array (BLKLOOPINIT). Uses the elaborated N_FANOUT_REPLICATORS.
genvar gk_si;
generate
  for (gk_si = 0; gk_si < N_FANOUT_REPLICATORS; gk_si++) begin : g_sorted_inputs
    always_ff @(posedge clk_gated or negedge rst_n)
      if (~rst_n) begin
        sorted_inputs[gk_si][0] <= '0;
        sorted_inputs[gk_si][1] <= '0;
      end else begin
        sorted_inputs[gk_si][0] <= sorted_inputs_c[0];
        sorted_inputs[gk_si][1] <= sorted_inputs_c[1];
      end
  end
endgenerate
