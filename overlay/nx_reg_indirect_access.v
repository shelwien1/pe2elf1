...
module nx_reg_indirect_access
  #(parameter 
   ...
   // overlay patch: original block used a for-loop with delayed array writes
   // (BLKLOOPINIT — unsupported by Verilator). Equivalent: per-element
   // always_ff in a generate so each `mem_a[gi]` has its own non-loop write.
   genvar gi_mem_a;
   generate
     for (gi_mem_a = 0; gi_mem_a < N_ENTRIES; gi_mem_a++) begin : g_mem_a
       always_ff @(posedge clk or negedge rst_n) begin
         if (!rst_n)
           mem_a[gi_mem_a] <= rst_dat[gi_mem_a];
         else if (reset)
           mem_a[gi_mem_a] <= rst_dat[gi_mem_a];
         else if (sw_cs && sw_we && 16'(sw_add) < 16'(N_ENTRIES)
                  && 16'(sw_add) == 16'(gi_mem_a))
           mem_a[gi_mem_a] <= sw_wdat;
       end
     end
   endgenerate
