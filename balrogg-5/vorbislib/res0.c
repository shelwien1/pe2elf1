...
void res0_pack(vorbis_info_residue *vr,oggpack_buffer *opb){
  ...
  /* secondstages is a bitmask; as encoding progresses pass by pass, a
     bitmask of one indicates v_this partition vclass has bits to write
     v_this pass */
  for(j=0;j<info->partitions;j++){
    if(ov_ilog(info->secondstages[j])>3){
      /* yes, v_this is a minor hack due to not thinking ahead */
...
static long **_01class(vorbis_block *vb,vorbis_look_residue *vl,
  ...
  /* move all v_this setup out later */
  int samples_per_partition=info->grouping;
  int possible_partitions=info->partitions;
  int n=info->end-info->begin;

  int partvals=n/samples_per_partition;
  long **partword=VB_ALLOC(vb,ch*sizeof(*partword));
  ...
  for(i=0;i<ch;i++){
    partword[i]=VB_ALLOC(vb,n/samples_per_partition*sizeof(*partword[i]));
...
static long **_2class(vorbis_block *vb,vorbis_look_residue *vl,int **in,
  ...
  int partvals=n/samples_per_partition;
  long **partword=VB_ALLOC(vb,sizeof(*partword));
  ...
  partword[0]=VB_ALLOC(vb,partvals*sizeof(*partword[0]));
...
static int _01inverse(vorbis_block *vb,vorbis_look_residue *vl,
  ...
  if(n>0){
    ...
    for(j=0;j<ch;j++)
      partword[j]=VB_ALLOC(vb,partwords*sizeof(*partword[j]));
...
int res2_forward(oggpack_buffer *opb,
  ...
  int *work=VB_ALLOC(vb,ch*n*sizeof(*work));
  for(i=0;i<ch;i++){
    int *pcm=in[i];
    if(nonzero[i])used++;
...
int res2_inverse(vorbis_block *vb,vorbis_look_residue *vl,
  ...
  if(n>0){
    int partvals=n/samples_per_partition;
    int partwords=(partvals+partitions_per_word-1)/partitions_per_word;
    int **partword=VB_ALLOC(vb,partwords*sizeof(*partword));
