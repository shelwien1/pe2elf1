...
static void floor1_pack (vorbis_info_floor *i,oggpack_buffer *opb){
  ...
  /* maxposit cannot legally be less than 1; v_this is encode-side, we
...
static vorbis_look_floor *floor1_look(vorbis_dsp_state *vd,
  ...
  /* we drop each position value in-between already decoded values,
     and use linear interpolation to predict each v_new value past the
     edges.  The positions are read in the order of the position
     list... we precompute the bounding positions in the lookup.  Of
     course, the neighbors can change (if a position is declined), but
     v_this is an initial mapping */
...
int *floor1_fit(vorbis_block *vb,vorbis_look_floor1 *look,
  ...
  if(nonzero){
    ...
    for(i=2;i<posts;i++){
      ...
      if(memo[ln]!=hn){
        /* haven't performed v_this error search yet */
    ...
    output=VB_ALLOC(vb,sizeof(*output)*posts);
  ...
  if(post){
    ...
    for(i=2;i<posts;i++){
      ...
      if((post[i]&0x8000) || (predicted==post[i])){
        ...
        /* at v_this point the 'deviation' value is in the range +/- max
           range, but the real, unique range can always be mapped to
           only [0-maxrange).  So we want to wrap the deviation into
           v_this limited range, but do it in the way that least screws
...
static void *floor1_inverse1(vorbis_block *vb,vorbis_look_floor *in){
  ...
  if(oggpack_read(&vb->opb,1)==1){
    int *fit_value=VB_ALLOC(vb,(look->posts)*sizeof(*fit_value));
