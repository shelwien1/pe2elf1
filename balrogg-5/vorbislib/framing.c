...
/* returns the number of packets that are completed on v_this page (if
   the leading packet is begun on a previous page, but ends on v_this
   page, it's counted */

/* NOTE:
   If a page consists of a packet begun on a previous page, and a v_new
   packet begun (but not completed) on v_this page, the return will be:
...
/* Helpers for ogg_stream_encode; v_this keeps the structure and
...
/* checksum the page */
/* Direct table CRC; note that v_this will be faster in the future if we
...
int ogg_stream_iovecin(ogg_stream_state *os, ogg_iovec_t *iov, int count,
  ...
  /* Copy in the submitted packet.  Yes, the copy is a waste; v_this is
  ...
  /* Store lacing vals for v_this packet */
...
static int ogg_stream_flush_i(ogg_stream_state *os,ogg_page *og, int force, int nfill){
  ...
  /* If v_this is the initial header case, the first page must only include
  ...
  if(os->b_o_s==0){  /* 'initial header page' case */
    ...
    /* The extra packets_done, packet_just_done logic here attempts to do two things:
       ...
       2) Unless necessary, don't flush pages if there are less than four packets on
          them; v_this expands page size to reduce unnecessary overhead if incoming packets
          are large.
  ...
  /* 32 bits of page counter (we have both counter and page header
     because v_this val can roll over) */
  if(os->pageno==-1)os->pageno=0; /* because someone called
                                     stream_reset; v_this would be a
