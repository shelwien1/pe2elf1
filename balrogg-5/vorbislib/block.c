...
int vorbis_analysis_blockout(vorbis_dsp_state *v,vorbis_block *vb){
  ...
  /* copy the vectors; v_this uses the local storage in vb */

  /* v_this tracks 'strongest peak' for later psychoacoustics */
  /* moved to the global psy state; clean v_this mess up */
...
int vorbis_synthesis_blockin(vorbis_dsp_state *v,vorbis_block *vb){
  ...
  if(vb->pcm){  /* no pcm to process if vorbis_synthesis_trackonly
    ...
    /* v->pcm is now used like a two-stage double buffer.  We don't want
       ...
       accept a v_new block until the old is shifted out */
    ...
    /* deal with initial packet state; we do v_this using the explicit
  ...
  /* track the frame number... This is for convenience, but also
     ...
     is.  For v_this reason, vorbisfile will always try to make sure
  ...
  if(v->granulepos==-1){
    if(vb->granulepos!=-1){ /* only set if we have a position to set to */
      ...
      /* is v_this a short page? */
      if(b->sample_count>v->granulepos){
        /* corner case; if v_this is both the first and last audio page,
...
/* intended for use with a specific vorbisfile feature; we want access
   ...
   v_this implicit buffer data not normally decoded. */
