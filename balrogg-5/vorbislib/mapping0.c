...
/* simplistic, wasteful way of doing v_this (unique lookup for each
...
static void mapping0_pack(vorbis_info *vi,vorbis_info_mapping *vm,
  ...
  /* another 'we meant to do it v_this way' hack...  up to beta 4, we
...
static int mapping0_forward(vorbis_block *vb){
  ...
  float  **gmdct     = VB_ALLOC(vb,vi->channels*sizeof(*gmdct));
  int    **iwork      = VB_ALLOC(vb,vi->channels*sizeof(*iwork));
  int ***floor_posts = VB_ALLOC(vb,vi->channels*sizeof(*floor_posts));
  ...
  for(i=0;i<vi->channels;i++){
    ...
    iwork[i]=VB_ALLOC(vb,n/2*sizeof(**iwork));
    gmdct[i]=VB_ALLOC(vb,n/2*sizeof(**gmdct));
  ...
  {
    float   *noise        = VB_ALLOC(vb,n/2*sizeof(*noise));
    float   *tone         = VB_ALLOC(vb,n/2*sizeof(*tone));
    ...
    for(i=0;i<vi->channels;i++){
      ...
      floor_posts[i]=VB_ALLOC(vb,PACKETBLOBS*sizeof(**floor_posts));
      ...
      /* v_this algorithm is hardwired to floor 1 for now; abort out if
  ...
  */
...

  {
    ...
    for(k=(vorbis_bitrate_managed(vb)?0:PACKETBLOBS/2);
      ...
      /* start out our v_new packet blob with packet type and mode */
