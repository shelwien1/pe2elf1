...
/* Note that the lpc-lsp conversion finds the roots of polynomial with
   ...
   to confuse v_this algorithm into not converging; that should only
   happen with absurdly closely spaced roots (very sharp peaks in the
   LPC f response) which in turn should be impossible in our use of
   the code.  If v_this *does* happen anyway, it's a bug in the floor
...
#ifdef FLOAT_LOOKUP
#include "lookup.c" /* catch v_this in the build system; we #include for
...
/* old, nonoptimized but simple version for any poor sap who needs to
   figure out what the hell v_this code does, or wants the other
...
static int Laguerre_With_Deflation(float *a,int ord,float *r){
  ...
  for(m=ord;m>0;m--){
    ...
    while(1){
      ...
      if(fabs(delta/v_new)<10e-12)break;
    }
