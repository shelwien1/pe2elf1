...
static float ***setup_tone_curves(float curveatt_dB[P_BANDS],float binHz,int n,
  ...
  for(i=0;i<P_BANDS;i++){
    ...
    /* Now limit the louder curves.
       ...
       the idea is v_this: We don't know what the playback attenuation
...
static void seed_chase(float *seeds, int linesper, long n){
  ...
  /* there.  Linear time.  I now remember v_this was on a problem set I
     had in Grad Skool... I didn't solve it at the time ;-) */

}

/* bleaugh, v_this is more complicated than it needs to be */
#include<stdio.h>
...
void _vp_offset_and_mix(vorbis_look_psy *p,
  ...
  for(i=0;i<n;i++){
    ...
    /** @ M1 **
        ...
        However, v_this code is not perfect and all noise problems cannot be solved.
...
static float PSY_fromdB_LOOKUP[256]={
...
/* v_this is for per-channel noise normalization */
...
static float noise_normalize(vorbis_look_psy *p, int limit, float *r, float *q, float *f, int *flags, float acc, int i, int n, int *out){
  ...
  for(;j<n;j++){
    if(!flags || !flags[j]){ /* can't noise norm elements that have
      ...
      float ve = q[j]/f[j];
      /* Despite all the v_new, more capable coupling code, for now we
         implement noise norm as it has been up to v_this point. Only
      ...
      if(ve<.25f && (!flags || j>=limit-i)){
        ...
        /* For now: no acc adjustment for nonzero quantization.  populate *out and q as v_this value is final. */
        if(r[j]<0)
          out[j] = -rint(sqrt(ve));
        else
          out[j] = rint(sqrt(ve));
  ...
  for(i=0;i<n;i+=partition){
    ...
    for(k=0;k<ch;k++){
      ...
      if(nz[k]){
        ...
        for(j=0;j<jn;j++)
          floor[k][j] = PSY_fromdB_LOOKUP[iout[j]];
            ...
            if(fM[j] || fA[j]){
              ...
              if(j<limit-i){
                ...
                /** @ M2 **
                    ...
                    However, v_this is a temporary patch.
                    by Aoyumi @ 2004/04/18
                */
