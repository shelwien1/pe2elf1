...
ogg_uint32_t *_make_words(char *l,long n,long sparsecount){
  ...
  /* sb_bitreverse the words because our bitwise packer/unpacker is LSb
     endian */
...
static ogg_uint32_t sb_bitreverse(ogg_uint32_t x){
...
int vorbis_book_init_decode(codebook *c,const static_codebook *s){
  ...
  if(n>0){
    ...
    if(n==1 && c->dec_maxlength==1){
      ...
      c->dec_firsttablen=ov_ilog(c->used_entries)-4; /* v_this is magic */
      ...
      for(i=0;i<n;i++){
        if(c->dec_codelengths[i]<=c->dec_firsttablen){
          ogg_uint32_t orig=sb_bitreverse(c->codelist[i]);
         ...
         hints for the non-direct-hits */
      {
        ...
        for(i=0;i<tabn;i++){
          ...
          if(c->dec_firsttable[sb_bitreverse(word)]==0){
               ...
               just drops), encode as the difference from the extremes. */
            {
              ...
              c->dec_firsttable[sb_bitreverse(word)]=
...
/* Unit tests of the dequantizer; v_this stuff will be OK
