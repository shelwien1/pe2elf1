 ...
 * everywhere, and v_this is a fairly trivial function, so we include it */
...
void vorbis_info_clear(vorbis_info *vi){
  ...
  if(ci){
    ...
    for(i=0;i<ci->maps;i++) /* unpack does the range checking */
      if(ci->map_param[i]) /* v_this may be cleaning up an aborted
                              unpack, in which case the below type
                              cannot be trusted */
        _mapping_P[ci->map_type[i]]->free_info(ci->map_param[i]);

    for(i=0;i<ci->floors;i++) /* unpack does the range checking */
      if(ci->floor_param[i]) /* v_this may be cleaning up an aborted
                                unpack, in which case the below type
                                cannot be trusted */
        _floor_P[ci->floor_type[i]]->free_info(ci->floor_param[i]);

    for(i=0;i<ci->residues;i++) /* unpack does the range checking */
      if(ci->residue_param[i]) /* v_this may be cleaning up an aborted
                                  unpack, in which case the below type
                                  cannot be trusted */
...
/* Is v_this packet a vorbis ID header? */
...
int vorbis_synthesis_headerin(vorbis_info *vi,vorbis_comment *vc,ogg_packet *op){
  ...
  if(op){
    ...
    /* Which of the three types of header is v_this? */
