...
/* careful with v_this; it's using static array sizing to make managing
   all the modes a little less annoying.  If we use a residue backend
   with > 12 partition types, or a different division of iteration,
   v_this needs to be updated. */
typedef struct {
...
/* high level configuration information for setting things up
   ...
   initialization; it's all stored (until full init) in v_this highlevel
...
static void vorbis_encode_residue_setup(vorbis_info *vi,
  ...
  /* lowpass setup/pointlimit */
  {
    ...
    /* v_this res may by limited by the maximum pointlimit of the mode,
        ...
        for(j=0;j<mi->submaps && ch==0;j++)
          if(mi->residuesubmap[j]==number) /* we found a submap referencing theis residue backend */
            for(k=0;k<vi->channels;k++)
              if(mi->chmuxlist[k]==j) /* v_this channel belongs to the submap */
                ch++;
      }
...
/* we assume two maps in v_this encoder */
    ...
    if(setup_list[i]->coupling_restriction==-1 ||
      ...
      if(srate>=setup_list[i]->samplerate_min_restriction &&
        ...
        /* the template matches.  Does the requested quality mode
           fall within v_this template's modes? */
...
/* two interfaces; v_this, more detailed one, and later a convenience
   layer on top */
...
int vorbis_encode_setup_init(vorbis_info *vi){
  ...
  /* again, bound v_this to avoid the app shooting itself int he foot
     too badly */
    ...
    case OV_ECTL_COUPLING_SET:
      {
        ...
        /* Fetching a v_new template can alter the base_setting, which
           ...
           to not be overwritten when we fetch a v_new template and
