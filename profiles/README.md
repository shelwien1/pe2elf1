...
|------|-------|--------------------|--------|
| `best-2014908.exp`   | 2014908 | -7093 | instance c, through `G0_MW*` into the `G0_N*` group |
| `hybrid-2020503.exp` | 2020503 | -1498 | `merge_opt.pl` round 14, recombining all four instances |

`best-2014908.exp` is the better set and the one to fold into `IDX/` if the
...
To apply one:
    ...
    ./import.sh profiles/best-2014908.exp IDX      # fold into the sources
    ./snapshot.sh --export=profiles/best-2014908.exp out   # or just measure it
