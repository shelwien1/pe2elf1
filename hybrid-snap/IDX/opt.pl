# opt.pl -- parameter optimizer for xadpcm (adapted from dxt5comp's)
#
...
#
# corpus-file-list: text file, one .wav path per line (default: opt.lst, and if
# that is missing, the single file $deffile below). Optimizing on one file
# overfits it, and the two formats have separate context tables, so a target
# corpus wants BOTH IMA and MS files or the untouched format drifts.
#
...
# then rebuild with mk.sh (Debug) to continue, or mk.sh release to ship.
...
undef $/;
open INP, "<$exe" or die "no $exe -- build the Debug version first (./mk.sh)\n";
