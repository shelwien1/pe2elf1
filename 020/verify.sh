...
#
# Var_Passion_oboe_melody.wav is here as a REGRESSION case, not for its size.
# It is the file that showed SGN_MAXNB had been swept down to a value that could
# not name a real header residual, so the encoder coded a capped magnitude and
# then went on using the true one -- see the note above sgn_maxnb in
# xad_codec.inc.  wavs2 had the same fault; the oboe file is the one that shows
# it 63 blocks in, on a single block, which is why it earns a line of its own.
# Nothing about that bug is visible in a size column: a truncating encoder is
# SMALLER (wavs2 was 1001 bytes smaller while undecodable), so only the
# round-trip half of check() can catch it, and only if the file is listed here.
sizes="wavs2:::1248346 wavs2:-s::1248876 wavs2:-ss::1248876
       ...
       Var_Passion_oboe_melody.wav:::486128
       Var_Passion_oboe_melody.wav:-s::486128
       Var_Passion_oboe_melody.wav:-ss::486128"
