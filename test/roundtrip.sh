...
# the directory given) through mtf2ima c and d, and check that d reproduces the
# input byte for byte and that ffmpeg accepts what c produced.
#
...
ok=0; bad=0; insize=0; outsize=0; metasize=0
...
while IFS= read -r f; do
    ...
    if cmp -s "$f" "$TMP/p.back" && ffmpeg -nostdin -v error -i "$TMP/p.out" -f s16le -y "$TMP/p.pcm" 2>"$TMP/err"; then
        ok=`expr $ok + 1`
...
echo "$ok ok, $bad failed; $insize bytes in, $outsize repacked, $metasize metainfo"
[ $bad = 0 ]
