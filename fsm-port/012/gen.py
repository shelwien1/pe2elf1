...
class Interp:
  ...
  OPF = ['GREG', 'RGB', 'XMM', 'MM', 'SREG', 'SSE_OS', 'MMG', 'GREGd', 'GREGq']
  ...
  def operand_file(self, tmpl, var):
    ...
    m = re.search(r'(greg|gregd|gregq|rgb|sreg|ssereg|mmxg)\[([^\]]*\$' + var + r'[^\]]*)\]', tmpl)
    if not m:
      return 'GREG'
  ...
  def build_insn(self):
    ...
    for lhs, rhs in self.insn_rules_raw():
      ...
      if imml == ['imm32', 'imm16']:
        ...
        continue
      has_g = bool(re.search(r'(greg|gregd|gregq|rgb|sreg|ssereg|mmxg)\[[^\]]*\$g[^\]]*\]', rhs))
      # operand form
      two_ops = (',' in rhs)
...
def emit(c, interp, out_path):
  ...
  w("enum OperandFile { OPF_GREG=0, OPF_RGB, OPF_XMM, OPF_MM, OPF_SREG, OPF_SSE_OS, OPF_MMG, OPF_GREGd, OPF_GREGq };\n\n")
