...
while( <I1> ) {
  s/\r//g;
  # Hold any trailing comment aside and put it back after the rewrite.  Every
  # match regex below captures to end of line, so without this the first import
  # would strip every annotation out of the .idx sources.
  $cmt = (s/(\s*\#.*?)[ \t]*$//) ? $1 : '';
  $eol = s/\n$// ? "\n" : '';
  if( /#(.*)$/ ) {
  } elsif( /^\s*Prefix\s+([^\s]*)/ ) {
