# opt.pl -- parameter optimizer for bmpc (adapted from auto_opt2.pl)
#
...
# image overfits it; 5-20 assorted bmp files is a much better target -- and
# they have to cover 8/24/32bpp, or whichever depth is missing drifts while
# nothing measures it. tools/bmpcrop.py cuts the big ones down so a pass
# costs seconds rather than minutes.
#
# exe: which binary to patch (default ./bmpc). Point it at a copy while
# tuning, then fold export.!!! back into MOD/sh_model-B0_h.inc.
#
...
undef $/;
open INP, "<$exe" or die "no $exe -- build it first (./gc.sh bmpc)\n";
