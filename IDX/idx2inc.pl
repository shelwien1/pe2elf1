...
while( <I1> ) {
  s/\r//g;
  s/\#(.*)$//g;
  # Trailing whitespace left behind by a stripped comment would otherwise be
  # swallowed by the greedy (.*) of the Number pattern and read as extra
  # zero bits -- every space is (' '&1)==0 -- silently multiplying the seeded
  # value by a power of two.  Rate/Rate1 and the mapping forms anchor with
  # \s*$ and never saw it; Number does not.
  s/\s+$//;
