...
(`MOD/sh_model-S0_p.inc`).  With `S0_W = 0` and the mixer frozen at `w = 1` the
stage is transparent and the coder reproduces the pre-SSE stream byte for
byte (344899 / 309703), which is the regression check for the whole path.
