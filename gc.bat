...
rem ---------------------------------------------------------------------------
rem ISPC backend: the same generated rc_kernel.cl, compiled into the exe.
rem No OpenCL runtime, no JIT, no -k cache; rc_ispc.cpp is the host side.
rem
...
%ispc% --target=avx512skx-x16,avx2-i32x16 --arch=x86-64 -O2 ^
 -DRC_ISPC=1 -DRCNUM=16 -DSCALElog=15 -DhSCALE=16384 ^
 -DLOWBYTES=8 -DCODBYTES=4 -DRC_LOWSPLIT=1 -DBLKFULL=65536 ^
 -DRC_CL_WORDOUT=1 -DRC_CL_BLOCKREAD=0 -DRC_RANGE64=0 -DRC_RENORM_TAIL=0 ^
 rc_kernel.cl -o rc_kernel_ispc.obj -h rc_kernel_ispc.h
...
rem which CL_* backend rc_cl.cpp / rc_ispc.cpp compile as -- exactly one.
rem RC_ISPC_THREAD=0 drops <thread> from rc_ispc.cpp (the worker becomes a
rem synchronous call in CL_Collect) if the static MSVC STL link objects to it.
set backend=-DRC_OPENCL=0 -DRC_ISPC=1
rem set backend=-DRC_OPENCL=1
...
%gcc% -s -std=c++23 -Ofast -O3 -fpermissive -Wno-format %arch% %incs% %opts% -static "-D__DIRNAM__=%DIRNAM%" coder.cpp FSM.cpp misc/model0.cpp misc/model1.cpp misc/timer.cpp rc_cl.cpp rc_ispc.cpp rc_kernel_ispc.obj rc_kernel_ispc_avx512skx.obj rc_kernel_ispc_avx2.obj -o coder.exe 

rem the OpenCL build, as before: set backend=-DRC_OPENCL=1 above, then
rem %gcc% -s -std=c++23 -Ofast -O3 -fpermissive -Wno-format %arch% %incs% %opts% -static "-D__DIRNAM__=%DIRNAM%" coder.cpp FSM.cpp misc/model0.cpp misc/model1.cpp misc/timer.cpp rc_cl.cpp rc_ispc.cpp ../OpenCL.lib -o coder.exe 
